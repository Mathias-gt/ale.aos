...
**Run arbitrary commands on an Alcatel-Lucent Enterprise AOS device**
...
Synopsis
--------
- Sends an arbitrary set of commands to an AOS node and returns the results read from the device.  This module includes an argument that will cause the module to wait for a specific condition before returning or timing out if the condition is not met.
    ...
    <table  border=0 cellpadding=0 class="documentation-table">
        <tr>
            ...
            <tr>
                ...
                <td>
                        <div>The commands to send to the remote AOS device. The resulting output from the command is returned.  If the <em>wait_for</em> argument is provided, the module is not returned until the condition is satisfied or the number of <em>retries</em> has been exceeded.</div>
...
.. note::
   - Tested against Alcatel-Lucent Enterprise AOS 8.9R03
...
.. code-block:: yaml
    ...
    - name: run show version and check to see if output contains Alcatel-Lucent Enterprise
      ale.aos.aos_command:
    ...
    - name: run a command that requires a confirmation. Note that prompt
      ...
      ale.aos.aos_command:
        commands:
          - command: reload from workin no rollback-timeout
            prompt: "Confirm Activate (Y/N) :"
            answer: Y
            newline: true

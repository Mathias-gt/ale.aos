...
short_description: Manage Alcatel-Lucent Enterprise AOS configuration sections
description:
- Alcatel-Lucent Enterprise AOS configurations use a simple block indent file syntax for segmenting configuration
  into sections.  This module provides an implementation for working with AOS configuration
  sections in a deterministic way.  This module works with CLI.
version_added: 1.0.0
notes:
- Tested against Alcatel-Lucent Enterprise AOS TBD
...
options:
  ...
  src:
    ...
    - The I(src) argument provides a path to the configuration file to load into the
      ...
      and correct diff. Alcatel-Lucent Enterprise AOS device config has 3 spaces indentation.
    type: path
  before:
    description:
...
commands:
  ...
  returned: always
  type: list
  sample: ['system name "switch01"', 'interfaces port 1/1/1 admin-state enable']
updates:
  description: The set of commands that will be pushed to the remote device
  returned: always
  type: list
...
from ansible_collections.ale.aos.plugins.module_utils.network.aos.aos import (
    get_config,
    get_connection,
    load_config,
    run_commands,
...
def save_config(module, result):
    ...
    if not module.check_mode:
        ...
        cmd = {
            "command": "copy running certified",
            "output": "text",
        }
        run_commands(module, [cmd])
    else:
        module.warn(
            "Skipping command `write memory flash-synchro` and `copy running certified` "
...
def main():
    ...
    if module.params["save_when"] == "always":
        ...
        output = run_commands(
            module,
            [
                {"command": "show configuration snapshot", "output": "text"},
                {"command": "cat working/vcsetup.cfg", "output": "text"},
            ],
        )
